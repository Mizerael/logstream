...
def task4() -> None:
    ...
    for i in range(len(dictionary.get("Имя")) - 1):
        ...
        if (
            surname == "Никитин"
            or surname[:-1] == "Никитин"
            and surname.endswith("а")
        ) and dictionary.get("Город проживания")[i] == "Москва":
            dictionary.get("Город проживания")[i] = "Махачкала"
        if (
            name == "Александр"
            and surname == "Иванов"
            and dictionary.get("Статус")[i] == "Студент"
        ):
    ...
    birthdates = list(
        zip(
            dictionary.get("Год рождения"),
            dictionary.get("Месяц рождения"),
            dictionary.get("Число рождения"),
        )
    )
    pres = [4, 2, 2]
    updated_dict["Дата рождения"] = [
        "-".join((f"{vl:0{pres[i]}d}" for i, vl in enumerate(tp)))
        for tp in birthdates
    ]
